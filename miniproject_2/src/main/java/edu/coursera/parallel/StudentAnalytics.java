...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
            final Student[] studentArray) {
    	double result=Arrays.asList(studentArray).parallelStream().filter(s->s.checkIsCurrent()==true).mapToDouble(s->s.getAge()).average().getAsDouble();
    	return result;
    }
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
    	return Arrays.asList(studentArray).parallelStream().filter(s->s.checkIsCurrent()==false).collect(Collectors.groupingBy(s->s.getFirstName(),Collectors.counting())).entrySet().stream().max(Comparator.comparing(Entry::getValue)).get().getKey();
		/*
		 * String mostCommon = null; Long mostCommonCount = -1; Map<String, Long>
		 * map=Arrays.stream(studentArray).filter(s->s.checkIsCurrent()==false).collect(
		 * Collectors.groupingBy(s->s.getFirstName(),Collectors.counting())); for
		 * (Map.Entry<String, Long> entry : map.entrySet()) { if (mostCommon == null ||
		 * entry.getValue() > mostCommonCount) { mostCommon = entry.getKey();
		 * mostCommonCount = entry.getValue(); } }
		 * 
		 * return mostCommon;
		 */
     ...
     * who are also older than 20 years old. A failing grade is anything below 
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
    	return (int)Arrays.asList(studentArray).parallelStream().filter(s->(!s.checkIsCurrent() && s.getAge() > 20 && s.getGrade() < 65)).count();
        //return (int)Arrays.stream(studentArray).filter(s->(!s.checkIsCurrent() && s.getAge() > 20 && s.getGrade() < 65)).count();
    	//throw new UnsupportedOperationException();
    }
}
